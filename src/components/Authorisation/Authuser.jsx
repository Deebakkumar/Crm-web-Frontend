...
import "./Authuser.css";
import logo from "../../Assets/images/React-icon(64).png";
...
import { auth, facebookprovider, githubprovider, provider } from '../../firebase';
...
const AuthUser = () => {
    ...
    const handleSignIn = (e) => {
        ...
        if (email === "" || password === "") {
            ...
            signInWithEmailAndPassword(auth, email, password, username).then((res) => {
                console.log(res)
                navigate("/")
                setLoading(false)
            }).catch((error) => {
        ...
        <div className="Auth container-fluid">
            <div className="Auth col">
                ...
                <div className='Login-cards-container'>
                    ...
                    <div onClick={handleSignInWithFacebook} className="Login-card2">
                        <div className="Login-card-body">
                            ...
                            {/* Toogle between login and register screens */}
                            {
                                register ? (
                                    ...
                                    <form>
                                        ...
                                        <Link onClick={handleForgotPassword} className='link'><p className='ForgotPassword'>{resetlink ? "Email sent. Please check spam folder" : "Forgot Password"}</p></Link>
                                    </form>
                                )
                            }
                            {/* Toogle between login and register pages */}
                            <h6 className='NewUserReg'><Link className='link' onClick={() => setRegister(!register)}> {register ? "Login" : "New User? Register Here"}</Link></h6>
                        </div>
                    </div>
                </div>
            </div>
